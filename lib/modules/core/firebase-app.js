...
export default class FirebaseApp {
  ...
  _staticsOrModuleInstance<M: FirebaseModule, S:FirebaseStatics>(statics: S, InstanceClass: Class<M>): FirebaseModuleAndStatics<M, S> {
    const getInstance = (url?: string): M => {
      ...
      if (_name === '_database' && Boolean(url)) {
        if (!this._namespaces[_name][url]) {
          this._namespaces[_name][url] = new InstanceClass(this, this._options, url);
        }
        return this._namespaces[_name][url]
      } else {
