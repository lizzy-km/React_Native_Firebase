...
export default class Database extends Base {
  constructor(firebase: Object, options: Object = {}) {
    ...
    if (firebase.options.persistenceCacheSizeBytes) {
      this._setPersistenceCacheSizeBytes(firebase.options.persistenceCacheSizeBytes);
    }
  ...
  /**
   * Set the max cache size in bytes
   * @param bytes
   * @private
   */
  _setPersistenceCacheSizeBytes(bytes: number) {
    this.log.debug(`Setting cache size of ${bytes} bytes.`);
    return promisify('setPersistenceCacheSizeBytes', FirebaseDatabase)(bytes);
  }
